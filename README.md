...
- 지원 기능: 현재 상태 입력, 잔금/예산 차감 전환형 입력, 총 매수원금/평단 전환형 입력, 오늘 주문 생성 모달, 계산 요약, 경고 표시, 최근 50개 주문 스냅샷 저장
- 가격 데이터: yfinance로 생성한 `public/market-data/*.json` 일봉을 기본 소스로 사용, 화면에서 일봉/주봉/월봉/년봉 집계, TradingView Lightweight Charts 기반 봉차트, MA5/20/60, BB20, RSI14, 매수 기준일 선택 시 해당 종가 계산기 자동 적용
